...
class Model(object):
  ...
  def __init__(self, user_count, item_count, cate_count, cate_list, predict_batch_size, predict_ads_num):
    ...
    hist_i_cat,hist_i_raw,wt_i=attention_item(i_emb,h_emb,self.sl,hc,cate_count) #[B,C,H]
    print('hist_i_cat is {}'.format(hist_i_cat))
    print('hist_i_raw is {}'.format(hist_i_raw))

    
    # hist_i_raw =attention(i_emb, h_emb, self.sl)
    hist_i,wt_cat_i =attention(i_emb, hist_i_cat, tf.ones_like(self.sl)*cate_count)
    # print("hist_i",hist_i.get_shape().as_list())
    #-- attention end ---
    
    hist_i = tf.concat([hist_i,hist_i_raw], axis=-1)
    hist_i=tf.contrib.layers.layer_norm(hist_i)
    hist_i = tf.layers.batch_normalization(inputs = hist_i)
    hist_i = tf.reshape(hist_i, [-1, hidden_units * 2 ], name='hist_bn')
    hist_i = tf.layers.dense(hist_i, hidden_units * 2, name='hist_fcn')
    ...
    hist_j_cat,hist_j_raw,wt_j=attention_item(j_emb,h_emb,self.sl,hc,cate_count)
    
    #hist_j =attention(j_emb, h_emb, self.sl)
    hist_j,wt_cat_j=attention(j_emb,hist_j_cat,tf.ones_like(self.sl)*cate_count)
    #-- attention end ---
    hist_j = tf.concat([hist_j,hist_j_raw], axis=-1)
    hist_j=tf.contrib.layers.layer_norm(hist_j)
    hist_j = tf.layers.batch_normalization(inputs = hist_j)
    hist_j = tf.reshape(hist_j, [-1, hidden_units * 2], name='hist_bn')
    hist_j = tf.layers.dense(hist_j, hidden_units * 2, name='hist_fcn', reuse=True)
    ...
    self.wt_cat_i = wt_cat_i
    self.wt_cat_j = wt_cat_j
    self.wt_i = wt_i
    self.wt_j = wt_j
  ...
  def get_embedding(self,sess):
    item_emb_w,cate_emb_w = sess.run([self.item_emb_w,self.cate_emb_w])
    return item_emb_w,cate_emb_w
  ...
  def test(self, sess, uij):
    y_positive_labels = np.ones(len(uij[1]))
    y_negative_labels = np.zeros(len(uij[2]))
    return sess.run([self.logits,self.logits_negative,self.wt_cat_j,self.wt_cat_j,self.wt_i,self.wt_j,self.hist_i,self.hc,self.i,self.j], feed_dict={
        self.u: uij[0],
        self.i: uij[1],
        self.j: uij[2],
        self.y:y_positive_labels,
...
def attention_item(queries, keys, keys_length,cate_idx,cate_count):
  ...
  item_weights = tf.squeeze(outputs)
  eps=1e-10
  ...
  # print("outputs",outputs.get_shape().as_list()) 
  return a,outputs1,item_weights
