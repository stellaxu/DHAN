...
#tf.app.flags.DEFINE_string("max_epoch","","max epoch nums")
...
#with open('/Users/stella.xu/Documents/raw_data/dataset.pkl', 'rb') as f:
  ...
  # print("test_set",test_set[1],len(test_set))
...
def _eval(sess, model):
  ...
  for _, uij in DataInputTest(test_set, test_batch_size):
    ...
    #end=time.time()
    #print('Finish {}'.format(ct))
    #print('Finish {}, cost {}'.format(ct,end-start))
    ct=ct+1
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
  ...
  for _ in range(max_epoch):
    ...
    for _, uij in DataInput(train_set, train_batch_size):
      ...
      #    model.print_layer(sess,uij)

      if model.global_step.eval() % 1000 == 0:
        test_gauc, Auc, test_loss_sum = _eval(sess, model)
        print('Epoch %d Global_step %d\tTrain_loss: %.4f\tTest_loss: %.4f\tEval_GAUC: %.4f\tEval_AUC: %.4f\tBest_AUC: %.4f' %
              (model.global_epoch_step.eval(), model.global_step.eval(), test_loss_sum,
               loss_sum / 1000, test_gauc, Auc,best_auc))
