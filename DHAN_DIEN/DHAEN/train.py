...
def train(
    buckets,
        ...
        mid_voc = "mid_voc.pkl",
        cat_voc = "cat_voc.pkl",
        model_folder = "dnn_save_path/ckpt_noshuff",
        batch_size = 128,
        maxlen = 100,
        test_iter = 1000,
        save_iter = 1000,
    ...
    best_model_path = model_folder + model_type + str(seed)
    gpu_options = tf.GPUOptions(allow_growth=True)
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        train_data = DataIterator(train_file,buckets, uid_voc, mid_voc, cat_voc, batch_size, maxlen, shuffle_each_epoch=False)
        ...
        elif model_type =='DHAEN':
            model = Model_DHAEN(n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
        elif model_type =='HAN':
            model = Model_HAN(n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
        else:
            ...
            for src, tgt in train_data:
                try:
                    ...
                    if (iter % test_iter) == 0:
                        ...
                        print('test_auc: %.4f ----test_loss: %.4f ---- test_accuracy: %.4f ---- test_aux_loss: %.4f' % eval(sess, test_data, model, best_model_path))
                        print('best_auc:',best_auc,"time.time()",time.time()-start_time)
                        loss_sum = 0.0
                        accuracy_sum = 0.0
                        aux_loss_sum = 0.0
                    if (iter % save_iter) == 0:
                    ...
                    print("some error in train,continue")
                    continue
            lr *= 0.5
...
if __name__ == '__main__':
    ...
    random.seed(SEED)
    if sys.argv[1] == 'train':
        train(buckets="", model_type=sys.argv[2], seed=SEED)
    elif sys.argv[1] == 'test':
        test(buckets="", model_type=sys.argv[2], seed=SEED)
    else:
        print('do nothing...')
...
#     tf.app.flags.DEFINE_string("mode","","mode info")
#     tf.app.flags.DEFINE_string("modelType","","model type")
...
#     if FLAGS.mode == 'train':
#         train(buckets = FLAGS.buckets,model_type=FLAGS.modelType, seed=SEED,train_file = FLAGS.buckets+"local_train_splitByUser",
#         test_file = FLAGS.buckets+"local_test_splitByUser",
#         uid_voc = FLAGS.buckets+"uid_voc.pkl",
#         mid_voc = FLAGS.buckets+"mid_voc.pkl",
#         cat_voc = FLAGS.buckets+"cat_voc.pkl",
#         model_folder = FLAGS.checkpointDir)
#     elif FLAGS.mode == 'test':
#         test(buckets = FLAGS.buckets,model_type=FLAGS.modelType, seed=SEED,train_file = FLAGS.buckets+"local_train",
#         test_file = FLAGS.buckets+"local_test",
#         uid_voc = FLAGS.buckets+"uid_voc.pkl",
#         mid_voc = FLAGS.buckets+"mid_voc.pkl",
#         cat_voc = FLAGS.buckets+"cat_voc.pkl",model_folder = FLAGS.checkpointDir)
#     else:
#         print('do nothing...')
