...
class Model_DHAN(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False):
        super(Model_DHAN, self).__init__(n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE,
                                           ATTENTION_SIZE,
                                           use_negsampling)
        ...
        with tf.name_scope('Attention_layer'):
            ...
            # attention_output1 = tf.reduce_sum(attention_output1, 1)
            ...
            # attention_output_2 = second_attention(self.item_eb,attention_output, ATTENTION_SIZE, self.mask)
            att_fea = tf.reduce_sum(attention_output_2, 1)
            att_fea = tf.concat([att_fea,att_fea_item_attention], axis=-1)
            # att_fea=tf.contrib.layers.layer_norm(att_fea)
            # att_fea = tf.layers.batch_normalization(inputs = att_fea)
            tf.summary.histogram('att_fea', att_fea)
        inp = tf.concat([self.uid_batch_embedded, self.item_eb, self.item_his_eb_sum, self.item_eb * self.item_his_eb_sum, att_fea], -1)
        # Fully connected layer
        self.build_fcn_net(inp, use_dice=True)
...
class Model_HAN(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False):
        super(Model_HAN, self).__init__(n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE,
                                           ATTENTION_SIZE,
                                           use_negsampling)
